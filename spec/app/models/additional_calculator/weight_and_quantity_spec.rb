...
describe Spree::AdditionalCalculator::WeightAndQuantity do
  before(:all) do
    ...
    @calculator = create(:weight_and_quantity_calculator)
  end

  before(:each) do
    @weight_rate1 = build(:additional_calculator_rate_for_weight, :from_value => 0, :to_value => 100, :rate => 100)
    @weight_rate2 = build(:additional_calculator_rate_for_weight, :from_value => 100.1, :to_value => 500, :rate => 500)
    @qnty_rate1 = build(:additional_calculator_rate_for_qnty, :from_value => 1, :to_value => 10, :rate => 10)
    @qnty_rate2 = build(:additional_calculator_rate_for_qnty, :from_value => 11, :to_value => 20, :rate => 20)
    @qnty_rate3 = build(:additional_calculator_rate_for_qnty, :from_value => 21, :to_value => 50, :rate => 50)
    @empty_order = create(:order)
    @order_with_one_item = create(:order_with_one_item).reload
    @order_with_items_without_weight = create(:order_with_items_without_weight).reload
  end
  ...
  context "just weight" do
    ...
    context "items with specific weight" do
      before(:each) do
        @order = create(:order_with_one_item).reload
      end
  ...
  context "weight and qnty" do
    before(:each) do
      ...
      @qnty_rate2.save!
      @qnty_rate3.save!
      @order1 = create(:order_with_one_item).reload
      @order2 = create(:order_with_two_items).reload
    end
